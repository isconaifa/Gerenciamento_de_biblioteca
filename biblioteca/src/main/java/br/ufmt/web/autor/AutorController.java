...
public class AutorController {
    ...
    @GetMapping(path = "/{id}")
  public ResponseEntity<AutorResponse> getById(@PathVariable(name = "id") int id) {
    Optional<Autor> found = repository.findById(id);
    if(found.isPresent()){
      AutorResponse response = AutorResponse.from(found.get());
...
@PostMapping
public ResponseEntity cadastrar(@RequestBody AutorRequest request){
